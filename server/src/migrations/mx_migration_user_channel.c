...
static void create_table_for_sqlite3(void) {
    ...
    char *sql = NULL;
    char **fill = mx_model_user_channel_fill_table();
    ...
    asprintf(&sql, "CREATE TABLE IF NOT EXISTS %s ("
                         ...
                         "FOREIGN KEY(%s) REFERENCES %s(id) );",
                         mx_model_user_channel_name_table(),
                         fill[0],
                         fill[1],
                         fill[2],
                         fill[3],
...
static void delete_table_for_sqlite3(void) {
    ...
    mx_valid_sqlite3_open_db(rc, db);
    asprintf(&sql, "DROP TABLE IF EXISTS %s;", mx_model_user_channel_name_table());
...
void mx_migration_user_channel(bool status) {
    if ((mx_strcmp(mx_model_user_channel_database(), "sqlite3") == 0) && (status == true))
        create_table_for_sqlite3();
    else if ((mx_strcmp(mx_model_user_channel_database(), "sqlite3") == 0) && (status == false))
